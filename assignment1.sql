...
SELECT city, COUNT(first_name) AS number_of_customers
FROM sale.customer
GROUP BY city
...
SELECT order_id, SUM(list_price*quantity*(1-discount)) AS total_amount_of_order
FROM sale.order_item
GROUP BY order_id
...
SELECT TOP 1 order_id, AVG(list_price) AS maximum_average_product_price
FROM sale.order_item
GROUP BY order_id
...
--The number of rows are equal which is not surprise for us. But rows are in different order. This order is certained by the  ORDER BY in first part(left) --
--then second part(right which is after comma)(That is evaluated from left to write)--
--This means, for [ORDER BY brand_id, list_price DESC] expression, first sort by brand_id in ASCENDİNG order and if brand_ids are equal then sorted by --
--list_price in descending order.--
--But for [ORDER BY list_price DESC, brand_id] expression, first sort by list_price in DESCENDİNG order and if list_prices are equal then sorted by --
--brand_id in ascending order.--
